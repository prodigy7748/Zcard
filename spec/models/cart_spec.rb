...
RSpec.describe Cart, type: :model do
  describe "購物車基本功能" do
    ...
    it "每個 Cart Item 都可以計算它自己的金額（小計）" do
      cart = Cart.new
      p1 = Product.create(title: 'ruby', price:100)
      p2 = Product.create(title: 'diamond', price:400)

      3.times { cart.add_item(p1.id) }
      5.times { cart.add_item(p2.id) }

      expect(cart.items.first.total_price).to be 300
      expect(cart.items.last.total_price).to be 2000
    end
    ...
    it "可以計算整台購物車的總消費金額" do
      ...
      expect(cart.total_price).to be 2300
    end
    it "聖誕節的時候全面打 9 折" do
      cart = Cart.new
      p1 = Product.create(title: 'ruby', price:100)
      ...
      expect(cart.total_price).to eq 2070
    end
  end
